...
class ThemeBlockResolver
{
    ...
    public function resolve(string $type, array $settings = [], array $context = []): array
    {
        return match ($type) {
            ...
            'related_products' => ['products' => collect($context['relatedProducts'] ?? [])->take(min(12, max(1, (int) ($settings['limit'] ?? 4))))],
            'product_archive_header', 'product_listing_grid', 'archive_breadcrumbs', 'archive_header', 'archive_toolbar', 'product_filters', 'active_filters', 'archive_product_grid', 'archive_pagination', 'archive_empty_state' => $context,
