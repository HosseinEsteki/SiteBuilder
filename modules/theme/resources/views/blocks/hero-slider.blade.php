...
<section class="theme-hero-slider" data-theme-block="hero_slider" style="--hero-height:{{ $height }}px;--hero-overlay:{{ $overlay / 100 }}">
    @forelse($slides as $slide)<article class="theme-hero-slide">
        @if($slide['desktop_image'] ?? null)<picture>@if($slide['mobile_image'] ?? null)<source media="(max-width:639px)" srcset="{{ $asset($slide['mobile_image']) }}">@endif<img src="{{ $asset($slide['desktop_image']) }}" alt="" loading="{{ $loop->first ? 'eager' : 'lazy' }}" fetchpriority="{{ $loop->first ? 'high' : 'auto' }}"></picture>@endif
